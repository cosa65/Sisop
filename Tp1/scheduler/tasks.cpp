...
void TaskBatch(int pid, vector<int> params) {
    ...
    for (int i = 0; i < cant_bloq; ++i){
        bloq_moment[i] = rand() % (cpu_time - cant_bloq + 1);
    }
    std::sort(bloq_moment.begin(), bloq_moment.end());
    int cpu_usado = 0;
    for (int i = 0; i< cant_bloq; ++i){
        if (bloq_moment[i] - cpu_usado > 0) uso_CPU(pid, bloq_moment[i] - cpu_usado);
        uso_IO(pid, 1);
        cpu_usado = bloq_moment[i];
    }
    uso_CPU(pid, cpu_time - cpu_usado - cant_bloq);
