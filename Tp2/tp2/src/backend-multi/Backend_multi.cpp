...
unsigned int alto = -1;
vector<vector<RWLock> > letras_lock;
vector<vector<RWLock> > palabras_lock;
...
int main(int argc, const char* argv[]) {
    ...
    tablero_letras = vector<vector<char> >(alto, vector<char>(ancho, VACIO));
    letras_lock = vector<vector<RWLock> >(alto, vector<RWLock>(ancho, RWLock()));

    tablero_palabras = vector<vector<char> >(alto, vector<char>(ancho, VACIO));
    palabras_lock = vector<vector<RWLock> >(alto, vector<RWLock>(ancho, RWLock()));
    ...
    while (true) {
        ...
        if (comando == MSG_LETRA) {
            ...
            cout << "esfichavalida\n";
                ...
                cerr << 1 << endl;
                letras_lock[ficha.fila][ficha.columna].wlock();
                tablero_letras[ficha.fila][ficha.columna] = ficha.letra;
                letras_lock[ficha.fila][ficha.columna].wunlock();
                // OK
            cout << "esfichasdfhsakdjfvalida\n";
            ...
            for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
                cerr << 2 << endl;
                palabras_lock[casillero->fila][casillero->columna].wlock();
                tablero_palabras[casillero->fila][casillero->columna] = casillero->letra;
                palabras_lock[casillero->fila][casillero->columna].wunlock();
            }
...
int enviar_tablero(int socket_fd) {
    ...
    for (unsigned int fila = 0; fila < alto; ++fila) {
        for (unsigned int col = 0; col < ancho; ++col) {
            palabras_lock[fila][col].rlock();
            cerr << 3 << endl;
            char letra = tablero_palabras[fila][col];
            palabras_lock[fila][col].runlock();
...
void quitar_letras(list<Casillero>& palabra_actual) {
    for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
        cerr << 4 << endl;
        letras_lock[casillero->fila][casillero->columna].wlock();
        tablero_letras[casillero->fila][casillero->columna] = VACIO;
        letras_lock[casillero->fila][casillero->columna].wunlock();
    }
    palabra_actual.clear();
}
...
bool es_ficha_valida_en_palabra(const Casillero& ficha, const list<Casillero>& palabra_actual) {
    ...
    cout << 7 << endl;
    letras_lock[ficha.fila][ficha.columna].rlock();
    if (tablero_letras[ficha.fila][ficha.columna] != VACIO) {
        letras_lock[ficha.fila][ficha.columna].runlock();
        return false;
    }
    letras_lock[ficha.fila][ficha.columna].runlock();
    cout << 8 << endl;
